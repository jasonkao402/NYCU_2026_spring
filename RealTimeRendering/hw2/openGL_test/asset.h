...
public:
	...
	// get mesh entry.
	inline std::shared_ptr<Mesh> getMesh(const char* name) {
		return getItem(meshes, name);
	};

	// get mesh entry.
	inline std::vector<std::shared_ptr<Mesh>> getMeshes(std::vector<const char*>&& names) {
		return getItems(meshes, names);
	};
...
private:
	...
	// called by getFile
	void loadMesh(const fs::path& filePath);
	// called by reloadFile
	void reloadMesh(const fs::path& filePath);
