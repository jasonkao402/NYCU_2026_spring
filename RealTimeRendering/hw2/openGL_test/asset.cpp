...
void Asset::AssetController::loadMesh(const fs::path& filePath)
{
    auto newMesh = std::make_shared<Mesh>(filePath.string());
    meshes.emplace(filePath.filename().string(), newMesh);
}

void Asset::AssetController::reloadMesh(const fs::path& filePath)
{
    auto it = meshes.find(filePath.filename().string());
    if (it != meshes.end())
        it->second->reload();
}
