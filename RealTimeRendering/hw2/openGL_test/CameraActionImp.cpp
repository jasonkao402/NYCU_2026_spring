...
namespace CameraAction {
	...
	void _WASDControlMove::move(float deltaTime, Mouse& mouse)
	{
	...
	template <int BUTTON>
	void _DragControlView<BUTTON>::view(float deltaTime, Mouse& mouse)
	{
	...
	void mouseControlBoth::move(float deltaTime, Mouse& mouse)
	{
	...
	void FPS_Control::view(float deltaTime, Mouse& mouse)
	{
		instance->viewAngle -= vec2(mouse.cursorPos - mouse.previousCursorPos) * instance->mouseSpeed;
		instance->setDirection();
	}
